...
const Navbar = ({id}) => {
  return (
    <div className="w-[70px] h-full fixed left-0 top-0 flex flex-col justify-between border-r border-gray-200 px-4 py-3 z-10 xl:py-6 ">
        <a href="/#home">
            <span className="text-3xl font-semibold text-red-400">E</span>.<span className="block w-min rotate-90 origin-bottom text-semibold text-[12px] dark:text-white">Mintah</span>
        </a>
        <div className="flex flex-col gap-y-1  xl:gap-y-1 sm:gap-y-2">
            {navbarData.map((item , i) =>(
                ...
                <span className={`text-2xl group-hover:scale-125 xl:group-hover:scale-115 transition-all ${item.id === id ? "text-red-500 scale-110 xl:group-hover:scale-100" : "text-yellow-600 scale-100 xl:group-hover:scale-90"}`}>{item.icon}</span>
        ...
        <p className="flex item-center justify-center text-[13px] text-gray-500 mt-6">
