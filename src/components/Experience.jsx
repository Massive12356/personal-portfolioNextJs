  ...
  return (
    <div id="experience" className="relative py-20">
      ...
      <div ref={containerRef} className="relative flex flex-col items-center justify-center gap-y-10 lg:gap-y-20 py-10 w-full">
        ...
        <motion.div  initial={{scaleY: 0}} style={{scaleY: scrollY}} className="absolute top-0 left-1/2 transform -translate-x-1/2 w-1 h-full bg-gray-300 origin-top" ></motion.div>
              ...
              <div className="text-gray-800 dark:text-gray-100">
                ...
                <ul className="pl-2">
                  {data.experience.map((exp, idx) => (
                    <li key={idx} className="my-1 font-extralight">{exp}</li>
                  ))}
                </ul>
              </div>
              {/* Arrow Indicator */}
              <span className={`absolute top-20 lg:hidden ${i % 2 === 0 ? "left-[105%] rotate-0" : "right-[105%] rotate-180"} text-red-500`}>
                    {arrowLeftIcon}
               </span>
